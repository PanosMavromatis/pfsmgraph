...
**2026-09-16 — no ADR.** The score is ported, not chosen: recording the two-part score as Accepted would contradict PRD §8, which leaves that choice open. The symbol-axis rule follows from ADRs 0011 and 0017, and the local minimum is deferred. The `+inf`/`1e100` rule stays in `core.md`, next to the earlier `-1` → `+inf` decision it refines. The master plan's loop-design subgoal now warns that an acceptance rule subtracting totals gets `inf - inf = nan` when both models are impossible, and that `diff > k` rejects the pair while `not (diff <= k)` accepts it. `docs/api/hmm/` was checked and needs no change, since nothing new is exported.
