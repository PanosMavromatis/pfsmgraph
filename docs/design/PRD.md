...
- **Which description length scores the topology search.** Raised 2026-09-03, from [`arc-emission-hmm-handoff.md`](arc-emission-hmm-handoff.md) §4. The imported Lush implementation uses a **two-part code**, L(M) + L(D|M), with the model cost built from `int-code-length` (a universal code for integers) and `comb-code-length` (a log-binomial); parameter *counting* lives in L(M), and the (k/2)·log n form of that term is BIC. The alternative is a **refined, one-part code** — normalised maximum likelihood, or stochastic complexity: −log P(D | θ̂) + COMP(M), where the parametric-complexity term counts *distinguishable distributions* by Fisher-information volume rather than raw parameters, so two models with equal parameter counts can score differently.

  The question is live for this project specifically, not as general MDL theory, because of [ADR 0015](adr/0015-arc-emission-mealy-formulation.md). That record notes that an MDL score computed for a state-emission HMM is not a baseline for an arc-emission one, since the two differ in parameter count while describing the same distributions — but that is an argument about a *two-part* code. Under a refined code the penalty counts distinguishable distributions, so the discrepancy might narrow or vanish. Whether it does is exactly the kind of claim this project should not assert without checking. *(Reasoned, not verified.)*

  **What blocks answering it now:** exact NML for HMM and PFSM classes is intractable — it requires a sum over all possible data sequences. Stolcke & Omohundro, whose merge search is the nearest published relative of ours, used a Bayesian MAP score (≈ a two-part code) and explicitly declined full parameter integration on those grounds. The practical routes are BIC/MAP surrogates, or **factorised/sequential NML** built on the tractable multinomial case, which fits because each transition and emission distribution here *is* a multinomial. Revision `04-hmm-v0.3.0` reproduces the two-part code, since reproducing is what a migration means; its duty toward this question is only to keep the criterion a **seam** rather than inlining it into the search driver.
...
>
> **The ADRs are now authoritative for D1–D11.** Amend decisions there, not here. Two carry a non-`Accepted` status, both superseded on 2026-09-04 by **ADR 0018**, which makes the build backend family-wide — meson-python for all five members: **ADR 0012**, which had been `Accepted (temporary)` and was expected to expire when the first `.pyx` landed, and **ADR 0008**, whose D7 and D8 it overrides. The `.pyx` never landed; 0012's premise was falsified instead, since the meta-path finder that forced it is injected by the editable install rather than by compilation. **This supersedes §6.1 and the D8 row in §2**, both of which are left as written — they record what was decided in 2026-06, and this section already establishes that the ADRs win where the two disagree. **ADR 0010** was `Proposed` on the same footing until 2026-09-01, when the encoder API reconciliation landed with the `dataseq` merge exactly as the prerequisite at the end of this section requires; it is now `Accepted` and records the settled API.
