...
- [x] Implement phase 4 (Numba CUDA), settling contraction behaviour, held bit-exact on a device
  > **Done:** Phase 4 landed in `7be0fc0`, completing forward-backward's lifecycle: `_forward_backward_cuda.py`, byte for byte with the reference on an NVIDIA L4, and `baum_welch(backend="cuda")` public. Contraction is settled: NVVM fuses multiply-add by default, so no launch both multiplies and adds a product. The benchmark confirmed no command is configured, and no timings were recorded.
  ...
  - [x] `/dp-compile:benchmark forward_backward` run for its report; if it confirms no benchmark command, record per-phase timings the way `viterbi_batch_speed.py` did, or say why not
    > **Note:** Ran 2026-09-15 after `7be0fc0`: the staleness gate passed with all four backends fresh and `cuda` available on the L4, and the command stopped at step 3 because `dp-compile.toml` defines no `[commands] benchmark`, as intended. **No per-phase timings were recorded on this branch**, by decision when the goal was closed: nothing here makes a speed claim, and phase 4's roughly eight launches per timestep are documented as a cost in its module rather than measured. A `forward_backward` counterpart to `.scratch/hmm-lush/measurements/viterbi_batch_speed.py` is the place for those numbers when they are wanted.
