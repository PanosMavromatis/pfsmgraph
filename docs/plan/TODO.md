...
- [ ] Implement Viterbi at ADR 0002 phase 2 (Cython), the first `.pyx` in a distribution. Backend equivalence against phase 1 is enforced by explicit differential tests in the suite's labelled non-shared section — **not** by ADR 0003's parameterized suite, which cannot be in force until `align`. Corrected 2026-09-09; see the phase-1 note above and `DEFERRED.md` under `align` acquiring a backend-selection API.
