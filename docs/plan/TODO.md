...
- [x] Review the whole `hmm` codebase and audit `docs/api/` against what 0.3.0 ships. Revision 03's audit (`docs/hmm-api-audit`, PR #39) covered the documentation alone. This one reads the code as well, because revision 04 is the first to add modules that no differential oracle checks: the split departs from the original on purpose (ADR 0022), and the search loop departs from its original on purpose. **Code:** read every module under `packages/pfsmgraph-hmm/src/pfsmgraph/hmm/` as a whole. Look for private helpers left without a consumer, which revision 04 is expected to *remove* from that list for `safe_divide`, so check that it did. Look for docstrings that still describe a subgoal's intermediate state, such as `_topology.py`'s "state merge next". Check that every ADR, `HMMLIB-ACCOUNT.md` and Lush line reference still points at what it names, and that no module presents new work as a port. Check the draw-order and `Generator` contracts end to end, from the public search call down to `_split_state`. Confirm the ADR 0003 backend header is unchanged from 0.2.0, and do not assume it. Check that the test counts, module count and ADR count in `core.md` match `uv run pytest` and the tree. **Docs:** check every name in `pfsmgraph.hmm.__all__` against its page, together with every parameter, return field and raised error of each public call. The search loop and training log export **nothing**: [ADR 0025](../design/adr/0025-topology-search-not-exported.md) keeps `__all__` at ten names at 0.3.0, so this check is against the same ten 0.2.0 shipped, and revision 04's four new modules are audited as private rather than as documented surface. Also check the `hmm/README.md` index, contracts and example against a package that now searches topology as well as trains; `docs/api/README.md`; and `packages/pfsmgraph-hmm/README.md`, which becomes the PyPI long description under an immutable version. [ADR 0013](../design/adr/0013-api-documentation-layout-and-tooling.md) still governs, so every code block is executed with its output pasted, and `tests/test_api_docs.py` stays the guard.
  ...
  > **Note:** three parts of this audit were discharged on `feat/hmm-search-export` before it ran. The export question is settled by [ADR 0025](../design/adr/0025-topology-search-not-exported.md), so this check no longer has to ask it. `docs/api/hmm/README.md`'s public-surface paragraph was repaired there — it enumerated six private modules and omitted all four revision 04 added — so that list can be confirmed rather than rebuilt. And `core.md`'s ADR count was corrected from twenty-four to twenty-five; the module count and the test count did not move, because that branch changed no code and no tests. What stays fully owed is everything about the modules themselves: docstrings still describing an intermediate state, the Lush and ADR references, and the draw-order contract end to end.
- [x] Release `pfsmgraph-hmm` 0.3.0. It follows 0.2.0's CI path — `just release-ci`, since `pfsmgraph-hmm` is in `ci_built_packages` and `just release` refuses it — and it is the first release to exercise that path from a version the tree bumped at opening rather than at the release commit. Expect **no** new ADR 0002 lifecycle phases: `HMMLIB-ACCOUNT.md` §12 records that not one topology-search method was compiled in the original, on the reasoning that "search *drives* compiled work — a split trial calls `run-converge`, which is compiled — so leaving the driver interpreted costs little". If that holds, 04 is the first `hmm` revision that adds no backend row, the `dp-compile` gate never arms, and the backend header is unchanged from 0.2.0's. Verify it rather than assuming it, and check the four-file release invariant plus `license-files` are still intact.
  > **Branch:** chore/release-hmm-0.3.0
  > **Done:** released 2026-09-18, tagged `pfsmgraph-hmm-v0.3.0` at `78f04a3`: twenty wheels and the sdist from run `35303614122`, all with PEP 740 attestations, and the sdist's digest predicted by a local build before the tag existed. §12 held: no new lifecycle phase or backend row, and the header byte-identical to 0.2.0's. It ships the search private under ADR 0025, and the release notes, the family's first `CHANGELOG.md`, say so first. `0.4.0.dev0` followed in the next commit, narrowing the version window to two commits. Found on the way: two public keyword additions a name-level check missed, and a `core.md` claim that a test pinned the tie-break, which a mutant disproved — PR #53
