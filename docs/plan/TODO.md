...
- [ ] Implement forward-backward in numpy as the reference: α, β, ξ, γ, in log space, with `+inf` bits for impossible events. State whether the base stays 2 — natural for the description lengths of revision 04, unusual everywhere else — or whether base *e* is used with a conversion at the DL boundary.
