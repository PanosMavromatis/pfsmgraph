...
- [x] Migrate the training log as **standard-output reporting only**: `update-training-log` and `training-log-line` (`hmm-trainer.lsh:457-477`), plus `save-training-log` (`hmm.lsh:166`). No GUI, no widget, no plotting dependency — see `DEFERRED.md`, `## Trigger: a second module needing training progress reporting`. The assumption is a Jupyter notebook whose output cell is browsed while the run continues, so the line format has to stay readable when hundreds of them accumulate in one cell.
...
- [x] Carry the forward recurrence through ADR 0002 phases 2 through 4 ([ADR 0016](../design/adr/0016-numba-cpu-parallel-phase.md) inserted a CPU-parallel phase 3, renumbering CUDA to phase 4), starting with a phase-0 `FORMALIZATION.md`. Settle whether the max-plus/logsumexp associative scan over time is worth implementing — wording kept from the draft on purpose: which semiring it should name depends on the base decision in the forward-backward subgoal, and `02-hmm-v0.1.0/_TODO.md` records why it was not silently corrected — or whether batch parallelism alone is the phase-3 answer; Viterbi's phase 3 found a per-timestep parallel region costs a flat fork/join per step, so the batch axis is the obvious candidate.
  ...
  > **Branch:** feat/hmm-forward-phases
  > **Done:** Forward-backward and the batched E-step carried through Cython, Numba CPU-parallel and Numba CUDA from a recovered `FORMALIZATION.md`, each phase byte for byte with the numpy reference and selectable as `baum_welch(backend=)`. Settled on the way: the parallel axis (each timestep's `(record, state)` cells), `-ffp-contract=off` for the extension, and NVVM's default multiply-add fusion, avoided on the device by never multiplying and adding in one launch — PR #38
- [x] Audit `docs/api/` for gaps before the release. Each subgoal documented what it touched, so the pages are accurate locally but have never been read as a whole against what 0.2.0 ships. Check every name in `pfsmgraph.hmm.__all__`, and every parameter, return field and raised error of each public call, against its page; the `hmm/README.md` index, contracts and example against a package that now trains as well as decodes; `docs/api/README.md`; and `packages/pfsmgraph-hmm/README.md`, which becomes the PyPI long description under an immutable version and so is the one surface that cannot be corrected after publishing. Include `dataseq`'s pages where revision 03 leaned on them, such as `pad_collate` feeding the batched trainer. [ADR 0013](../design/adr/0013-api-documentation-layout-and-tooling.md) still governs: every code block is executed and its output pasted, so `tests/test_api_docs.py` remains the guard against drift.
  > **Branch:** docs/hmm-api-audit
  ...
  > **Done:** `docs/api/`, the hmm README and its PyPI description read against the finished surface. The index described training nowhere and undercounted `baum_welch`'s backends; the README and description still called training forthcoming. `baum_welch`'s backend docstring was stale, `BackendName` is now defined on `backends.md`, `BaumWelchResult` and `BackendStatus` have field tables, five training/batching/backend contracts were added, and `container.md` states the mask/lengths and empty-record properties hmm relies on. `tests/test_api_docs.py` 11/11 — PR #39
- [x] Release `pfsmgraph-hmm` 0.2.0 — the first release with platform wheels, if the seam subgoal ships a public call that reaches a compiled kernel.
  > **Branch:** chore/release-hmm-0.2.0
  ...
  > **Note:** the docs audit (PR #39) left wheel shape unstated in the README on purpose. Once this subgoal settles it, check `backends.md`'s remedy for a missing extension ("install a platform wheel, or build from source") against what PyPI actually carries.
  > **Done:** 0.2.0 is on PyPI — twenty platform wheels over linux x86_64/aarch64, macOS arm64 and Windows x86_64 on cp310–cp314, plus an sdist, built by the repository's first CI and published through Trusted Publishing with PEP 740 attestations. The conditional in this subgoal resolved: `backend="cython"` is public, so the pure wheel could not continue. `just release` now refuses CI-built members and `release-ci` tags them; `preflight`'s `py3-none-any` assertion was re-examined and deliberately kept rather than relaxed, since no member publishes platform wheels locally. The verification found that the member `LICENSE` had never reached any `hmm` wheel, 0.1.0 included — fixed here with `license-files`. `backends.md`'s remedy was checked as this note asked and needs no change: "install a platform wheel" becomes actionable with this release. A `pip install` gets `manylinux_2_17_x86_64` with `cython ✓`, bit-exact with the numpy reference — PR #40
