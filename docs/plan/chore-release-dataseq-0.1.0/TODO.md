...
- [x] Sweep the prose claims about repository state, semantically
  ...
  - [x] `docs/agents/core.md` and `codex.md`, then regenerate the `AGENTS.*` artifacts
    ...
    > tells a reviewer to _report_ as staleness 100 lines earlier. Also two stale
  ...
  - [x] `docs/api/dataseq/` — every executed code block still matching its pasted output
    ...
    > drift. The finding is the _absence of a guard_, not a defect. Stock doctest fails 39
...
- [x] Set honest version lower bounds
  ...
  - [x] Confirm the workspace source is not what makes it resolve
    ...
    > `uv pip compile` on `pfsmgraph-dataseq>=0.1` fails: _"Because only
    > pfsmgraph-dataseq==0.0.0 is available and you require pfsmgraph-dataseq>=0.1, we can
    > conclude that your requirements are unsatisfiable."_ Unbounded, it resolves to the
...
- [x] Release: bump, build, publish, tag
  ...
  > refuses to unpack it: _"symlink destination for ../../LICENSE is outside of the target
  > directory"_. That is a **consumer-side** failure, caught here only because `uv build`
  > builds the wheel _through_ the sdist; a backend building both from the source tree
  ...
  > **Done:** `pfsmgraph-dataseq` 0.1.0 is on PyPI, tagged `pfsmgraph-dataseq-v0.1.0` at
  > `58e7617`, and verified from the published wheel rather than the local one -- a
  > `--no-project` install straight from PyPI imports `pfsmgraph.dataseq` and reports
  > `0.1.0`. The published metadata carries the corrected identity
  > (`ai.labs@panos-mav.dev`, `Mavromatis AI Labs`), `License-Expression: MIT` with
  > `License-File: LICENSE`, `Typing :: Typed`, and `numpy>=1.24` as its only dependency.
  > **The first attempt failed with a 403, and the cause was the `justfile`, not the
  > token.** `security add-generic-password`'s interactive `-w` prompt truncates its input
  > at 128 characters silently -- measured: 200 in, 128 back, no error and a zero exit --
  > so `token-set` stored a well-formed prefix of a valid token and PyPI could only report
  > `Invalid or non-existent authentication information`. Both setters now read the token
  > with the shell's own `read -rs` and verify the round-trip, deleting the entry rather
  > than leaving a broken credential; `-U` makes a rotation reuse the same command. The
  > failure cost nothing -- the 403 happens before any bytes are accepted, so `0.1.0` was
  > never consumed.
  ...
  - [x] Add a README and a LICENSE file to `pfsmgraph-dataseq` and make the sdist carry them, not `.gitignore`
    ...
    > surface where drift is _immutable_ was the one surface unguarded. Discovery now also
  ...
  - [x] `uv build --package pfsmgraph-dataseq` and inspect both artifacts before anything leaves the machine
    ...
    > **The path was the whole question.** The advice named the _distribution_ root,
  ...
  - [x] Publish to PyPI — the user runs this; it is irreversible
  ...
  - [x] Tag `pfsmgraph-dataseq-v0.1.0` by hand and push the tag
