...
- [x] Audit the `dataseq` pages revision 03 leans on
  > **Note:** worked unattended at the user's request; the one question was answered with the recommendation.
  > **Q:** `hmm`'s batched calls rely on two `pad_collate` properties that `container.md` never stated: `mask` can be rebuilt from `lengths`, and a batch of empty records has width 0. Should the base layer's page name its consumer while stating them?
  > **A (recommendation, adopted):** yes. ADR 0013 makes these pages normative for the seams between distributions. A property a sibling relies on is exactly such a seam, and naming `hmm` tells a future editor of `pad_collate` who would break.
  - [x] `pad_collate` and the container pages agree with how the batched trainer and decode use them
    > **Note:** `viterbi_batch` and `baum_welch` call `pad_collate` on slices of their records and pass their kernels `codes` and `lengths` only. Both derive liveness from `lengths` and never read `mask`, which is sound only while the two agree. Everything the pages already said held. What was missing was a statement of the properties `hmm` relies on.
    > **Done:** `container.md` gains two sections, each with an executed example: `mask` and `lengths` always agree, and empty records collate to zero width. The first names `hmm` as the consumer. Both properties were already tested in `dataseq`, by `test_mask_and_lengths_agree` and `test_all_empty_sequences`, so the pages now state what the tests already enforced. The dataseq README had two lines from before revision 03. "Will make the Cython and CUDA backends … mechanical" now points at `hmm`'s kernels, which exist. "none of which have anything to do with deep learning" had become false in spirit once `hmm` gained a `torch` backend; it now says none of them needs torch to run, and that `hmm`'s torch backend is an optional extra.
  - [x] `uv run pytest tests/test_api_docs.py` is green
    > **Done:** 11 of 11 after every goal of this plan, and after this one.
