...
- [ ] Report search progress on **standard output** by porting the original training log, which is this revision's rather than revision 03's: `update-training-log` and `training-log-line` (`hmm-trainer.lsh:457-477`) append one line per *accepted* move from `keep-model` (`677-690`), holding the size, the split (`n ^`) or merge (`i v j`) marker, `data-dl`, `model-dl`, `total-dl` and `d`, every one of which only `_mdl.py` and the search driver produce. Drop `test-data-dl`, which the original never assigned (all seven logged lines read `0`); the three tracked `_training_log` files are the format's oracle. Decide whether rejected moves are reported too, since the original records only accepted ones and the search rejects most. Revision 03's `baum_welch(..., log=)` already reports each fit's convergence checks, so a search's output nests those under its moves unless it silences them. Persisting the log beside a saved model is `DEFERRED.md`'s model-persistence entry. This is the run a user most wants to watch and the one most likely to prompt a dashboard; the decision is that it does not get one — see `DEFERRED.md`, `## Trigger: a second module needing training progress reporting`.
