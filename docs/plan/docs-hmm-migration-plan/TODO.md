...
"Subgoal 2 of goal 3, `feat-dataseq-merge`" — the `dataseq` source was read _inside_ the
...
- [x] Draft the three revision plans and register them in the master plan
  ...
  - [x] Write the three drafts as `docs/plan/planned/<label>.md`, carrying the `## Subgoals` section `/close-revision` archives, so opening one is a splice
  - [x] Add a `## Planned revisions` section to `docs/plan/TODO.md`, and amend its "tracks only work that is active now" claim, which the section falsifies
    > **Done:** three drafts under `docs/plan/planned/`, each carrying a
...
- [x] Open revision 02 using `/open-revision` and hand off
  - [x] Splice `docs/plan/planned/02-hmm-v0.1.0.md` from `## Subgoals` down into `docs/plan/TODO.md`, leaving its `**Status**: planned` preamble behind, and `git rm` the draft
  - [x] Move revision 02's entry out of `## Planned revisions`
  - [x] Confirm `/new-branch` can take the first subgoal cleanly — a revision is open, so the next branch backlinks to it rather than running standalone as this one did
    > **Found:** `/open-revision` refused the label outright — `refused: docs/plan/02-hmm-v0.1.0 already exists`. Its guard infers "opened before" from the directory, which is sound in the designed lifecycle because only `/file-plans` creates one; the pre-drafting convention written earlier on this branch broke that inference and made a free label look taken. Fixed by relocating the drafts to `docs/plan/planned/<label>.md`, which leaves every command's assumptions intact. **The `**Status**: planned` preamble could not have prevented this** — the tooling reads the filesystem, not the file — which is worth remembering the next time a convention is defended by a marker inside a file.
    > **Found:** the splice boundary in the drafts was also wrong, and the `git rm` is what exposed it. The falsifier block sat in the preamble, which "stays behind", while subgoal 1 says "check the three falsifiers **above**" — deleting the draft would have left that pointing at nothing. The falsifiers now travel with the subgoals. `03` and `04` carry the same shape and need the same treatment when they are opened.
