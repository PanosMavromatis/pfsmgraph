...
Training has five ([baum_welch.md](baum_welch.md)). The first four are lifecycle phases
...
| `"cuda"` | the same passes in Numba CUDA, one device thread per cell | the `gpu` extra and a CUDA device |
