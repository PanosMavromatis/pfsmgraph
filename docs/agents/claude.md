...
`_forward_backward_cython.pyx`, `_forward_backward_cpu_parallel.py` and
`_forward_backward_cuda.py` on 2026-09-15, so all eight are on disk. The paths come from the
