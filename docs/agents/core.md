...
measured at `S >= 2`. `_suggest_d` stays as the reproduction, and the pinning test is to
be rewritten to show the switch when the scan lands. **It also qualifies the `+inf` decision above**: Brent
