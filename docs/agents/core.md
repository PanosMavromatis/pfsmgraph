...
`-ffp-contract=off`. `tests/test_forward_backward_backends.py` (158 tests) compares on
...
`tests/test_baum_welch_backends.py` (314 tests, the kernel half also on `cython`, `cpu_parallel` and `torch-cuda`) pins both, with constructed subnormal arcs and
...
viterbi_batch python ✓ · cython ✓ · cpu_parallel ✓ · cuda ✓ | forward_backward python ✓ · cython ✓ · cpu_parallel ✓ | baum_welch python ✓ · cython ✓ · cpu_parallel ✓ · torch ✓` on a machine with a GPU and `… · cuda ✗ (no CUDA device
...
- `docs/design/algorithms/<algorithm>/FORMALIZATION.md` — the language-agnostic
  ...
  gives `-log2 Σ init_p`, and records that the kernel's docstring claims otherwise.
