...
`tests/test_baum_welch_backends.py` (122 tests) pins both, with constructed subnormal arcs and
...
Beside it is `baum_welch(params, records, *, backend="python", ...)`, the EM loop, public
since 2026-09-14 (the private `_em` until `feat/hmm-torch-backend`), whose `backend=` selects
the per-record E-step and whose result is `BaumWelchResult`: it sums counts over **records** before each M-step, so the
...
dynamic-programming kernel**. Nine names are exported from
...
and `baum_welch` and `BaumWelchResult` since later that day.
...
forward_backward python ✓ | baum_welch python ✓ · torch ✓` on a machine with a GPU and `… · cuda ✗ (no CUDA device
