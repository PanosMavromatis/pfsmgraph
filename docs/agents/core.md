...
`hmm/_viterbi.py` reached ADR 0002 phase 1, and **gained its first compiled row on
2026-09-09** with `_viterbi_cython.pyx` at phase 2: `BACKENDS` holds
`Backend("python", "pfsmgraph.hmm._viterbi")` and
...
- `docs/design/algorithms/<algorithm>/FORMALIZATION.md` — the language-agnostic
  ...
  its tie-breaking rule is contract, and its `Parallel decomposition` section is where
  "undetermined" is recorded as an answer rather than a gap.
