...
own `_TABLE["viterbi_batch"]` key (phases 1 and 2 so far; the Cython batch finishes one record before starting the next, and is held to the numpy batch exactly). It carries δ unchanged at a padded
...
viterbi_batch python ✓ · cython ✓ | forward_backward python ✓ | baum_welch python ✓ · torch ✓` on a machine with a GPU and `… · cuda ✗ (no CUDA device
