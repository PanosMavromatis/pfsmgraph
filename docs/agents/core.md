...
is therefore bit-exact with phase 1. **Phases 1-3 are not bit-identical to each other
everywhere, contrary to their own docstrings**: phase 1 uses numpy's vectorised `log2` and
phases 2-3 libm's scalar one, which differ by one ulp in about 0.1% of inputs on an AVX-512
host (measured 2026-09-13), so `total_bits` can sit an ulp apart and a near-tie could in
principle flip. Their exact tests pass because their seeds avoid it; unifying the
logarithm is in `docs/plan/DEFERRED.md`. `_numeric.py` is the numeric
