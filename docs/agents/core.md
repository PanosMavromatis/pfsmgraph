...
contract whose count depends only on the state count. `tests/test_topology.py` holds its 540 tests:
...
`begin` at 0.9 fails when that fix is reverted. The twelfth is `_mdl.py`, the minimum description length criterion revision 04 scores topology
