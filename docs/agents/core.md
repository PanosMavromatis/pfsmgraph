...
- `docs/plan/TODO.md` — the master plan. Its `## Planned revisions` section holds revisions drafted but not yet opened, one directory each under `docs/plan/NN-label/` in the same shape `/close-revision` leaves behind, so opening one is a splice from its `_TODO.md` rather than an authoring job. A planned revision's directory is indistinguishable from a closed one on disk, which is why each carries a `**Status**: planned` preamble and why the registry entry is load-bearing.
