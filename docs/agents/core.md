...
own `_TABLE["viterbi_batch"]` key (phase 1 only so far). It carries δ unchanged at a padded
step and reads the final argmin from column `L` for every row, which is what makes the
carry observable: a padded step taken fails 11 of its 28 tests, while ψ written at padded
...
viterbi_batch python ✓ | forward_backward python ✓ | baum_welch python ✓ · torch ✓` on a machine with a GPU and `… · cuda ✗ (no CUDA device
