...
`tests/test_baum_welch_backends.py` (182 tests, the kernel half also on `torch-cuda`) pins both, with constructed subnormal arcs and
...
revision 04's `_mdl.py`. `tests/test_baum_welch.py` holds its 87 tests. **`tests/test_hmmlearn_oracle.py` checks the
...
own `_TABLE["viterbi_batch"]` key (phases 1-3 so far; the Cython batch finishes one record before starting the next, the numba batch runs `prange` over each timestep's `(record, state)` cells, and both are held to the numpy batch exactly). It carries δ unchanged at a padded
...
viterbi_batch python ✓ · cython ✓ · cpu_parallel ✓ | forward_backward python ✓ | baum_welch python ✓ · torch ✓` on a machine with a GPU and `… · cuda ✗ (no CUDA device
