...
Beside it is `_em(params, records, ...)`, the EM loop, private until the backend-seam
subgoal decides a public shape: it sums counts over **records** before each M-step, so the
...
revision 04's `_mdl.py`. `tests/test_baum_welch.py` holds its 78 tests. **`tests/test_hmmlearn_oracle.py` checks the
...
agree to 3e-15 against its `scaling` implementation. **`_em` itself cannot be compared**,
...
division and `_em`'s zero-count restore to `test_baum_welch.py`. The sixth is `_forward_backward.py`, the project's second dynamic-programming kernel, written 2026-09-14 on
...
`.scratch/hmm-lush/measurements/`. Those are tracked so a decision record's evidence can
