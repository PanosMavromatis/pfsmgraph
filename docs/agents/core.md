...
contract whose count depends only on the state count. `tests/test_topology.py` holds its 645 tests:
...
independent generator in the contract's order, which a same-seed comparison alone would not catch. The twelfth is `_mdl.py`, the minimum description length criterion revision 04 scores topology
