...
contract whose count depends only on the state count. `tests/test_topology.py` holds its 528 tests:
properties rather than a differential oracle, since the split departs from the original on purpose, exact
`==` wherever the arithmetic allows, mutation-checked against five defects including the `:172` one. The twelfth is `_mdl.py`, the minimum description length criterion revision 04 scores topology
