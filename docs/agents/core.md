...
`-ffp-contract=off`. `tests/test_forward_backward_backends.py` (103 tests) compares on
...
`tests/test_baum_welch_backends.py` (250 tests, the kernel half also on `cython` and `torch-cuda`) pins both, with constructed subnormal arcs and
...
viterbi_batch python ✓ · cython ✓ · cpu_parallel ✓ · cuda ✓ | forward_backward python ✓ · cython ✓ | baum_welch python ✓ · cython ✓ · torch ✓` on a machine with a GPU and `… · cuda ✗ (no CUDA device
