...
that log a partial oracle for moves, though not for bits. `tests/test_search.py` holds its 32
...
identity alone. Acceptance is `_search`'s. Every trial also takes `score_backend=` beside `backend=`, since `forward_backward` has no `torch` phase for the scoring and ADR 0021 forbids substituting one. **A merge round
...
120-330 times faster than numpy's on the corpus with bit-identical scale factors. Its 32 tests pin the composition, not the parts,
...
`tests/test_baum_welch_backends.py` (380 tests, the kernel half also on `cython`, `cpu_parallel`, `cuda` and `torch-cuda`) pins both, with constructed subnormal arcs and
