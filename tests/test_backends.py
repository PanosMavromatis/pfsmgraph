...
def test_viterbi_has_all_four_phases_in_lifecycle_order_and_forward_backward_two():
    ...
    # lifecycle on 2026-09-13; forward_backward reached phase 2 on 2026-09-15 with no
    ...
    assert {a: [r.name for r in rows] for a, rows in hmm_backends._TABLE.items()} == {
        ...
        "forward_backward": ["python", "cython"],
        "baum_welch": ["python", "cython", "torch"],
    }
...
def test_the_header_names_every_registered_backend():
    ...
    assert format_header(detect()) == (
        ...
        + cuda_cell
        + " | forward_backward python ✓ · cython ✓ | baum_welch python ✓ · cython ✓ · torch ✓"
    )
...
def test_header_actually_reaches_the_session_output(pytester):
    ...
    result.stdout.fnmatch_lines(["backends: viterbi python ✓*| forward_backward python ✓ · cython ✓ | baum_welch python ✓ · cython ✓ · torch ✓"])
