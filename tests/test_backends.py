...
def test_viterbi_has_all_four_phases_in_lifecycle_order_and_forward_backward_three():
    ...
    # lifecycle on 2026-09-13; forward_backward reached phase 3 on 2026-09-15 with no
    ...
    assert {a: [r.name for r in rows] for a, rows in hmm_backends._TABLE.items()} == {
        ...
        "baum_welch": ["python", "cython", "cpu_parallel", "torch"],
    }
...
def test_header_actually_reaches_the_session_output(pytester):
    ...
    result.stdout.fnmatch_lines(["backends: viterbi python ✓*| forward_backward python ✓ · cython ✓ · cpu_parallel ✓ | baum_welch python ✓ · cython ✓ · cpu_parallel ✓ · torch ✓"])
